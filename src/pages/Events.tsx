...
// --- Updated Helper Function to Calculate Single Next Date ---
/**
 * Calculates the very next upcoming fellowship date (either a 2nd or 4th Sunday).
 * After the day ends, it will automatically find the next one on page reload.
 * @returns {string} A formatted string for a single date, like "July 27, Sunday"
 */
const getUpcomingFellowshipDate = (): string => {
  // Finds all Sundays for a given month and year
  ...
  // Formats a date object into "Month Day, Weekday" format (e.g., "July 27, Sunday")
  const formatDate = (date: Date): string => {
    ...
    const parts = formatted.split(', '); // e.g., ["Sunday", "July 27"]
    return `${parts[1]}, ${parts[0]}`; // Returns "July 27, Sunday"
  };

  const now = new Date();
  let searchDate = new Date(); // Start searching from today
  ...
  for (let i = 0; i < 12; i++) {
    ...
    // Sort potential dates to check them in chronological order
    potentialDates.sort((a, b) => a.getTime() - b.getTime());

    // Find the first potential date that is in the future
    for (const date of potentialDates) {
      if (date > now) {
        return formatDate(date); // Found the next date, return it
      }
    }

    // Move to the first day of the next month for the next loop iteration
    searchDate = new Date(year, month + 1, 1);
  }

  return "Date to be announced"; // Fallback if no date is found
};
